...
export default function RoleSelection() {
  return <div className="role-page"><header className="role-header"><Link href="/" className="brand"><img src="/manus-storage/polaris-logo_7787b873.png" alt="" className="brand-mark" /><span>POLARIS</span></Link><p>ACCESS GATEWAY / SIH 2026</p></header><main className="role-main"><div className="role-intro"><p className="eyebrow"><span className="pulse-mark" /> Select an observation route</p><h1>Choose your<br /><em>POLARIS</em> experience.</h1><p>Every role works from the same evolving record of polar science. Trace the workflow from contribution to discovery.</p></div><div className="role-cards">{roles.map(({ title, route, label, icon: Icon, text, index, color }) => <Link href={route} className={`role-card ${color}`} key={title}><span className="role-index">{index}</span><Icon size={30} strokeWidth={1.35} /><div><p>{label}</p><h2>{title}</h2><span>{text}</span></div><b>Enter route <ArrowRight size={17} /></b></Link>)}</div></main><footer className="role-footer"><span><img src="/manus-storage/polaris-logo_7787b873.png" alt="" /> ONE GATEWAY TO POLAR SCIENCE</span><span>RESEARCH → EXPEDITION → MEDIA → EDUCATION</span></footer></div>;
}
